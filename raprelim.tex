...
\begin{table*}
  ...
  \begin{minipage}{.45\linewidth}
    %\centering
      ...
      $\wramm$ & \makecell{\PORF{}\\ \WRCoh{}} &$\rlxmm$-Acyclic & \makecell{\PORF{}\\ \RlxRCoh\\ \RlxWCoh} \\ 
      \hline
      $\ramm$ & \makecell{\PORF{} \\
...
There is also a stronger variant of $\rlxmm$ called $\rlxmm$-Acyclic which allows \PORF{} besides the coherence axioms of relaxed.  
...
\vspace{-\baselineskip}
\end{figure*}
\paragraph*{Comparing memory models.} The two lemmas gives the following partial order of the memory models reflecting the number of allowed behaviours: $\scmm\preccurlyeq\sramm\preccurlyeq\ramm\preccurlyeq\{\wramm,\{\rlxmm\text{-Acyclic}\preccurlyeq\rlxmm\}\}$ with models towards the right allowing more behaviors.  All models are weaker than $\scmm$. $\rlxmm$ is weaker than $\ramm$ but incomparable with $\wramm$. In particular, the lack of synchronization across $\rf$
